				...
				jQuery('.login.bb-login #login > h1 > a').each(function() {
					...
					if ( function_exists( 'buddypress' ) && isset( buddypress()->buddyboss ) ) {
						...
						if ( '0' === $enable_private_network ) {
							...
							$this.addClass('bb-login-title').append( '<span class="bs-cs-login-title private-on" style="display: flex; justify-content: space-between; align-items: center;"><div style="margin-left:-8rem;"><?php echo $rx_title; ?></div><div><img class="bs-cs-login-logo private-on" src="/wp-content/uploads/2024/07/logoR.png" style="width: 100%; max-width: 100px; margin-left:1.5rem;"/></div></span>' );
							$this.append( '<p style="display: flex; justify-content: space-between; align-items: center; line-height:1.25; font-size:1rem; color:#000!important; margin-left: -8rem; margin-right: -8rem; margin-top: 1rem; text-align: justify !important;">En esta Plataforma de Economía Circular podrás ofrecer y adquirir excedentes de materiales y productos para evitar su desperdicio y maximizar su aprovechamiento. Podrás vincularte con otras empresas que, como la tuya, están interesadas en aprovechar oportunidades económicas al mismo tiempo que contribuyen a cuidar el medio ambiente. <br/><br/>Si aún no estás registrado, ¡Súmate, conoce y aprovecha los beneficios que esta plataforma puede traer para tu empresa y el medio ambiente!</p>' );
							jQuery('#login h1 a span').unwrap();
							jQuery('#login h1 a p').unwrap();
