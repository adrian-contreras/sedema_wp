...
class Frontend {
	...
	public function output( $id, $title = false, $description = false ) { // phpcs:ignore Generic.Metrics.CyclomaticComplexity.TooHigh
		...
		//error_log('Frontend.php::post_content::'.$form->post_content);
		// Decode the form data.
		$form_data = wpforms_decode( $form->post_content );
		//error_log('Frontend.php::form_data::'.json_encode($form_data));
		//error_log('Frontend.php::form_data::'.$form_data['id']);
		//error_log('Frontend.php::form_data::'.$form_data['settings']['ajax_submit']);
		//error_log('Frontend.php::form_data::'.$form_data['settings']['ajax_submit_action']);
		//$form_data['settings']['ajax_submit_action']='mi_accion_ajax';
		//if ($form_data['id'] == 139) {
        //$form_data['settings']['ajax_submit'] = true;
        //$form_data['settings']['ajax_submit_action'] = 'mi_accion_ajax';
		...
		//error_log('Frontend.php::REQUEST_URI::'.$_SERVER['REQUEST_URI']);
		//error_log('Frontend.php::remove_query_arg::'.remove_query_arg( '' ));
		// Basic information.
		/**
		 ...
		 */
		//add_filter('wpforms_frontend_form_data', 'modificar_ajax_action', 10, 1);		 
		...
		//error_log('Frontend.php::form_atts::'.json_encode($form_atts));
		/**
	...
	public function render_field( $form_data, $field ) {
		//error_log('Frontend::render_field::$field'.json_encode($field));
		//error_log('Frontend::render_field::$field::type'.$field['type']);
		if($field['type']!='html' AND $field['type']!='file_upload'){
		 ...
		 */
		if($field['type']!='html' AND $field['type']!='file_upload'){ 
			//error_log('Frontend::render_field::$field'.$field['type']);
		...
		//error_log('Frontend::render_field::$attributes::'.json_encode($attributes));
		...
		//error_log('Frontend::render_field::$properties::'.json_encode($field['properties']));
		if($field['type']!='html' AND $field['type']!='file_upload'){
		/**
		 * Core actions on this hook:
		 * Priority / Description
		...
		//else {
			...
			if($field['type']=='html'){			
				if($field['code']=='[user_groups_feeds]'){
					...
					printf('<fieldset>');
					printf('<legend class="%1$s">%2$s</legend>',
						( $field['properties']['label']['class'] ),
						( $field['properties']['label']['value'] )
					);
